...
# ─────────────────────────────────────────────
#  S3 Policy RAG(s3_policy_rag.py)도 os.getenv만 보므로 AWS·버킷 설정을 함께 옮긴다.
#  이게 빠져 있으면 키를 넣어 둬도 기준 검색이 조용히 꺼진 채로 동작한다.
...
# ─────────────────────────────────────────────
# Policy 원문 다듬기
#  S3 청크는 [문서명]·[발행기관] 머리말 + PDF에서 그대로 긁은 본문이다.
#  그대로 4건을 깔면 화면이 글자로 덮여 정작 볼 문장이 묻힌다.
#  머리말은 떼고(표·캡션에 이미 있다), 본문에서 이 Claim과 겹치는 문장만 앞에 낸다.
# ─────────────────────────────────────────────
_PARTICLES = ("으로서", "으로", "에서", "에게", "이라", "라는", "까지", "부터",
              "보다", "처럼", "마다", "이나", "나", "와", "과", "을", "를",
              "이", "가", "은", "는", "에", "의", "로", "도", "만", "한", "할", "함")


def policy_body(raw: str) -> str:
    """청크에서 [문서명]·[발행기관] 같은 머리말을 떼고 본문만 남긴다."""
    if "[내용]" in raw:
        raw = raw.split("[내용]", 1)[1]
    lines = [ln for ln in raw.splitlines() if not re.match(r"\s*\[[^\]]+\]", ln)]
    return "\n".join(lines).strip()


def _reflow(body: str) -> list[str]:
    """PDF에서 긁힌 본문은 문장 중간에서 줄이 끊긴다. 문장 단위로 다시 잇는다."""
    out: list[str] = []
...
def _claim_keywords(claim: dict, criterion: dict) -> set[str]:
    """Claim에서 뽑은 대조용 낱말. 형태소 분석 대신 흔한 조사만 떼어 쓴다."""
    words: set[str] = set()
    for src in (claim.get("claim_text", ""), claim.get("claim_type", ""),
                claim.get("claim_scope", ""), criterion.get("시험항목", "")):
        for w in re.findall(r"[가-힣A-Za-z]{2,}", str(src)):
            words.add(w)
            for p in _PARTICLES:
...
def _policy_doc_freq() -> tuple[dict[str, int], int]:
    """색인 전체에서 각 낱말이 몇 개 문서에 나오는지 센다.

    '기준'·'제품'·'표시'처럼 거의 모든 고시에 나오는 말에 문장이 걸리면
    엉뚱한 줄이 뽑힌다. 낱말을 손으로 금지 목록에 넣는 대신 실제 빈도를 세서
    흔한 말은 가중치를 낮춘다.
    """
    try:
        docs = gl.policy_retriever.vectorstore.docstore._dict.values()
    except Exception:            # Policy RAG가 꺼져 있으면 가중치 없이 동작
        return {}, 0
    freq: dict[str, int] = {}
    total = 0
    for d in docs:
        total += 1
        for w in set(re.findall(r"[가-힣A-Za-z]{2,}", d.page_content)):
            freq[w] = freq.get(w, 0) + 1
    return freq, total


def _keyword_weight(word: str) -> float:
    """흔할수록 0에 가깝고 드물수록 큰 값. 색인이 없으면 전부 같은 무게."""
    freq, total = _policy_doc_freq()
    if not total:
        return 1.0
    import math
    return max(0.0, math.log(total / (1 + freq.get(word, 0))) - 1.0)


def _mark(line: str, keys: list[str], limit: int = 150) -> str:
    """걸린 낱말을 굵게 하고 너무 긴 줄은 줄인다. 전체는 아래 펼침에 그대로 있다."""
    if len(line) > limit:
        line = line[:limit].rstrip() + "…"
    if not keys:
        return line
    # 한 번에 치환한다. 낱말마다 replace를 돌리면 '인증'과 '인증을'이 겹쳐
    # 이미 넣은 표시 안에 또 표시가 들어간다(****인증**을**).
    pattern = re.compile("|".join(re.escape(k) for k in
                                  sorted(keys, key=len, reverse=True)))
    marked = pattern.sub(lambda m: f"**{m.group(0)}**", line)
    # 붙어 있는 두 낱말은 '**함량****기준**'이 되어 마크다운이 깨진다. 한 덩어리로 합친다.
    return marked.replace("****", "")


def policy_excerpt(raw: str, claim: dict, criterion: dict,
                   keep: int = 2) -> tuple[list[str], str, int]:
    """(보여줄 문장, 원문 전체, 걸린 문장 수). 겹치는 낱말이 없으면 빈 목록."""
    body = policy_body(raw)
    if not body:
        return [], "", 0
    lines = _reflow(body)
    weights = {k: _keyword_weight(k) for k in _claim_keywords(claim, criterion)}
    weights = {k: w for k, w in weights.items() if w > 0}
    if not weights:
        return [], "\n".join(lines), 0
    scored = [(sum(w for k, w in weights.items() if k in ln), i, ln)
              for i, ln in enumerate(lines)]
    hits = [s for s in scored if s[0] > 0]
    if not hits:
        return [], "\n".join(lines), 0
    top = sorted(sorted(hits, key=lambda s: -s[0])[:keep], key=lambda s: s[1])
    shown = [_mark(ln, [k for k in weights if k in ln]) for _, _, ln in top]
    return shown, "\n".join(lines), len(hits)
...
def evidence_form(key: str, product_ids: list[str]) -> list[dict]:
    ...
    with tab_file:
        ...
        if text:
            ...
            # 인증서·성적서에는 인증번호와 시험 수치가 같이 적혀 있는 일이 흔하다.
            # 인증번호가 있는 자료는 수치 대조 후보에서 빠지므로(_pick_numeric),
            # 그대로 내면 자료를 냈는데도 판정이 '수치 대조 자료 없음'에 머문다.
            # 직접 입력 탭에만 있던 안내를 여기서도 하고, 빼는 선택지를 준다.
            drop_cert = False
            if str(preview.get("cert_no", "")).strip() and str(preview.get("value", "")).strip():
                st.info("이 자료에는 인증번호와 수치가 함께 있습니다. 인증번호가 있는 자료는 "
                        "인증 확인 경로로 처리되어 **수치 대조에서는 제외**됩니다.", icon="🔀")
                drop_cert = st.checkbox(
                    f"수치 대조용으로 제출 (인증번호 `{preview['cert_no']}`는 빼고 보냅니다)",
                    key=f"{key}-file-dropcert")
            if st.button("이 PDF/TXT로 재검토", key=f"{key}-file-go", type="primary"):
                if drop_cert:
                    # 원문을 그대로 넘기면 core가 다시 파싱해 인증번호를 되살린다.
                    # 화면에서 확인한 항목을 구조화 레코드로 만들어 보낸다.
                    rec = {k: v for k, v in preview.items()
                           if k not in ("field_sources", "dropped_fields")}
                    rec["cert_no"] = ""
                    rec["evidence_id"] = f"SESSION-FILE-{datetime.now():%H%M%S}"
                    records.append(rec)
                else:
                    records.append({
...
def render_claim(idx: int, rec: dict) -> None:
    ...
    if not state["needs_human"]:
        ...
        with tabs[0]:
            ...
            if refs:
                ...
                # 표는 criteria_master에 실린 기준(기준값·버전·시행일)이 있을 때만 쓸모가 있다.
                # 검색이 데려온 고시 원문 청크만 있으면 문서명·페이지가 반복될 뿐이고,
                # 그 둘은 아래 원문 목록에 이미 붙어 있다.
                structured = [r for r in refs if gl.get_criterion(r.get("criterion_id", ""))]
                if structured:
                    enriched = []
                    for r in structured:
                    ...
                    table = table.loc[:, [c for c in table.columns
                                          if table[c].astype(str).str.strip().any()]]
                    st.dataframe(table, hide_index=True, width="stretch")

                st.markdown("**Policy 원문 — 이 Claim과 겹치는 문장**")
                off_topic = []
                for n, ref in enumerate(refs, 1):
                    ...
                    picked, full, n_hit = policy_excerpt(raw, claim, c)
                    if not picked:
                        off_topic.append((n, title, ref, full))
                        continue

                    st.caption(f"{n}. {title} · {ref.get('page','조항 미기재')}")
                    for ln in picked:
                        st.markdown(f"> {ln}")
                    rest = n_hit - len(picked)
                    label = ("원문 전체 보기" if rest <= 0
                             else f"원문 전체 보기 (겹치는 문장 {rest}건 더)")
                ...
                # 검색이 데려왔지만 이 Claim의 낱말과 하나도 겹치지 않는 문서다.
                # 버리지는 않되(검색 결과를 숨기면 추적이 끊긴다) 접어 둔다.
                if off_topic:
                    with st.expander(f"겹치는 문장이 없는 검색 결과 {len(off_topic)}건"):
                        st.caption("검색은 이 문서들도 데려왔지만 Claim과 겹치는 문장이 "
                                   "없습니다. 판정 근거로 쓰이지 않았습니다.")
                        for n, title, ref, full in off_topic:
                            st.markdown(f"**{n}. {title}** · {ref.get('page','조항 미기재')}")
                            st.code(full[:600] + ("…" if len(full) > 600 else ""),
                                    language=None)
                st.caption("신뢰등급이 VERIFIED가 아닌 기준은 단독 판정 근거로 쓰지 않습니다. "
                           "발췌는 Claim과 겹치는 낱말로 고른 것이며, 원문 전체는 펼쳐서 볼 수 있습니다.")
            else:
    ...
    # 제출했는데 판정이 그대로일 때 — 파일을 못 읽은 것과 구분해 준다.
    if note := rec.get("submit_note"):
        st.info(
            f"제출하신 자료(`{note['read']}`)는 정상적으로 읽었지만, 이 Claim에 필요한 "
            "내용이 그 문서에 없어 판정이 바뀌지 않았습니다.  \n"
            "필요한 자료: " + " · ".join(f"**{m}**" for m in note["asked"]) + "  \n"
            "예를 들어 인증서에는 인증 사실만 있고 함량·측정값은 없습니다. "
            "요청한 항목이 담긴 성적서·확인서를 올리거나, '직접 입력' 탭에 값을 옮겨 적어 주세요.",
            icon="📄",
        )
...
def submit_evidence(idx: int, records: list[dict]) -> None:
    ...
    before = rec["state"]
    asked = list((before.get("request") or {}).get("missing_evidence", []))
    rec["history"].append(dict(before))
    rec["state"] = gl.resume_review(
        rec["thread_id"],
        evidence=structured,
        evidence_text=text_rec["__text__"] if text_rec else None,
    )
    # 자료를 냈는데 판정이 그대로면 담당자는 앱이 파일을 읽지 못했다고 생각한다.
    # 실제로는 읽었지만 요청한 내용이 그 문서에 없는 경우가 대부분이다 —
    # 예: 함량 확인서를 요청했는데 인증서를 제출. 그 구분을 화면에 남긴다.
    rep = rec["state"].get("report") or {}
    if rep.get("status") == "INSUFFICIENT" and rep.get("missing_evidence") == asked and asked:
        rec["submit_note"] = {
            "asked": asked,
            "read": text_rec.get("file_name") if text_rec else "직접 입력한 항목",
        }
    else:
        rec.pop("submit_note", None)
    st.rerun()
